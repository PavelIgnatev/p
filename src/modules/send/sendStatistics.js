...
const sendMail = async (mail, tournaments, html, region, transporter) => {
  ...
  worksheet.columns = [
    ...
    { header: "Score1", key: "score" },
    { header: "Score2", key: "score2" },
  ];
  ...
  try {
    if (region) {
      console.log(
        `Начинаю создавать таблицу со статистикой игороков по региону: ${region}`
      );
    ...
    console.log(
      `Не удалось создать таблицу со статистикой игороков по региону: ${region}`
    );
  }
  ...
  for (let i = 0; i < 10; i++) {
    try {
      ...
      await promiseWrapper(
        mailOptions(mail, html, buffer, filename),
        transporter
      );
      break;
    } catch (e) {
...
const sendStatistics = async (errorTournaments, transporter) => {
  ...
  if (!aliases.length) {
    console.log(
      "Нечего отправлять, все сыграли правильные турниры",
      new Date()
    );
    return;
  }

  console.log("Начал отправлять статистику по турнирам на почты регионов");
  ...
  for (let key in errorTournamentsByRegion) {
    ...
    switch (key) {
      ...
      case "mgr.miranda85@gmail.com,pocarrlatina@gmail.com":
        region = "Latina";
        break;
    ...
    try {
      ...
      if (message?.flat()?.length && region) {
        ...
        console.log(
          `Закончил отправлять статистику по турнирам на почту ${key}`
        );
      } else {
        console.log(
          `Статистика по турнирам не отправлена, так как сообщение пустое`
        );
      }
    } catch (error) {
      console.log(
        `Отправка не письма на почту ${key} не удалась, произошла ошибка: `,
        error
      );
    }
  }

  console.log("Завершил отправлять статистику по турнирам на почты регионов");
