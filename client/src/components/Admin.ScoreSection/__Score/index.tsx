...
import { SCORES_TYPES, SCORES_TYPES_TO_FIELDS } from "../constants";
import { b } from "../index";
import {
  $scores,
  deleteScoresRequest,
...
import { scoresModel, scoreType } from "../../../@types/scoreModel";
...
type valuesType = Array<Record<string, number | string | scoreType[]>>;
...
export const ScoresSectionScores = (props: ScoresSectionScoresProps) => {
  const savedScores = useStore($scores) as Array<scoresModel[]>;
  const postPending = useStore(postScoresRequest.pending);
  const deletePending = useStore(deleteScoresRequest.pending);
  const aliases = useStore($allAliases);
  const pending = postPending || deletePending;

  const [types, setTypes] = useState<scoreType[]>(["Score", SCORES_TYPES[0]]);
  ...
  useEffect(() => {
    ...
    if (props.color === "orange") {
      setTypes([SCORES_TYPES[0]]);
    } else if (props.color !== "orange") {
      setTypes(["Score", SCORES_TYPES[0]]);
    }
  }, [props.color]);

  const handleAddscoreRow = () => {
    setTypes((types) => [...types, SCORES_TYPES[0]]);
    setValues((values) => [...values, {}]);
  };

  const handleRemoveScoreRow = (
    scoreIndex: number,
    exceptionIndex = 0,
    isExceptions = false
  ) => {
    if (isExceptions) {
      setValues((values) => {
        ...
        const { exceptions } = newValues[scoreIndex];
      ...
      setTypes((types) => {
        ...
        newTypes.splice(scoreIndex, 1);
        return newTypes;
      });
      setValues((values) => {
        const newValues = [...values];
        newValues.splice(scoreIndex, 1);
        return newValues;
      });
    }
  };
  ...
  const editableRule = types.map((type, ruleIndex) => ({
    type,
    values: new Array(SCORES_TYPES_TO_FIELDS[type].length)
      .fill(null)
      .map((_, i) => {
  ...
  const withoutScoreRule = (scoresTypes: scoreType[]) => {
    delete scoresTypes[9];
    return scoresTypes;
  };

  const scores: scoresModel[][] = [editableRule, ...savedScores];

  const handleSaveRule = () => {
    postScoresRequest([...editableRule]);
    setValues([{}]);
  };
  ...
  const handleTypeChange = (rowIndex: number) => (e: any) => {
    setTypes((types) => {
      const newTypes = [...types];
      newTypes[rowIndex] = e?.value ?? SCORES_TYPES[0];
      return newTypes;
    });
    setValues((values) => {
      const newValues = [...values];
  ...
  return (
    <div className={b("rules")}>
      {scores.map((scoreRows, scoreIndex) => {
        const isComposite = scoreRows.length > 1;
        const isEditable = scoreIndex === 0;

        const isSaveBtnDisabled = getIsSaveBtnDisabled(scoreRows, values);
        ...
        return (
          ...
          >
            ...
            {scoreRows.map((scoreRow, rowIndex) => {
              const {
                type: ruleType,
                values: ruleValues,
                exceptions,
              ...
              const isLastRow = rowIndex === scoreRows.length - 1;
                ...
                >
                  <div style={{ display: "flex", minHeight: "48px" }}>
                    <Select
                      ...
                      options={withoutScoreRule(SCORES_TYPES)?.map((type) => ({
                        value: type,
                        label: type,
                      }))}
                      ...
                      isDisabled={
                        !isEditable || ruleType === "Score" || pending
                      }
                    ...
                    {isEditable &&
                      scores[scoreIndex][rowIndex].type !== "UserName" && (
                        <BaseButton
                          className={b("rule-row-control-btn")}
                          onClick={() => {
                            const data = { ...scores[scoreIndex][rowIndex] };
                            if (data["exceptions"]) {
                    ...
                    {isEditable && ruleType !== "Score" && (
                      <BaseButton
                        className={b("rule-row-control-btn")}
                        onClick={() => handleRemoveScoreRow(rowIndex)}
                        disabled={pending}
                      >
                        Х
                      </BaseButton>
                    ...
                    {!isEditable && isLastRow && (
                      <Popconfirm
                        ...
                        cancelText="No"
                        onConfirm={() =>
                          deleteScoresRequest(scores[scoreIndex]).then(() => {
                            setTypes(scores[scoreIndex].map((e) => e.type));

                            setValues(
                              scores[scoreIndex].map(
                                ({ values, exceptions }) => {
                                  const result: any = {};
                    ...
                    {!isEditable && isLastRow && (
                      <BaseButton
                        className={b("rule-row-control-btn", { delete: true })}
                        onClick={() => deleteScoresRequest(scores[scoreIndex])}
                        disabled={pending}
                        red
                      >
                        Delete rule
                      ...
                      >
                        <div>
                          {exception.map((exc, indexExc) => {
                            ...
                            const fields = SCORES_TYPES_TO_FIELDS[
                              ruleType
                            ] as Field[];
                        ...
                        {isEditable && (
                          <BaseButton
                            className={b("rule-row-control-btn")}
                            onClick={() =>
                              handleRemoveScoreRow(rowIndex, indexExc2, true)
                            }
                            disabled={pending}
                            style={{ height: "48px" }}
                          >
...
const getIsSaveBtnDisabled = (ruleRows: scoresModel[], values: valuesType) => {
  for (let rowKey = 0; rowKey < ruleRows.length; rowKey += 1) {
    if (!values[rowKey]) {
      return true;
    }
    const fieldKeysCount = SCORES_TYPES_TO_FIELDS[ruleRows[rowKey].type].length;
        ...
        for (
          ...
          for (
            ...
            const fieldKeysCount =
              // @ts-ignore
              SCORES_TYPES_TO_FIELDS[values[rowKey].exceptions[dd][rr].type]
                .length;
  ...
  return ruleRows.length === 0 || ruleRows.length === 1;
};
